<?php
global $post;
$lawgit_por_area_social = get_post_meta( $post->ID, 'lawgit_por_area_social', true );
$lawgit_practice_enable_social = get_post_meta( $post->ID, 'lawgit_practice_enable_social', true );

$postUrl = 'http' . ( isset( $_SERVER['HTTPS'] ) ? 's' : '' ) . '://' . "{$_SERVER['HTTP_HOST']}{$_SERVER['REQUEST_URI']}";
?>
<div class="practice-single-meta-img">
    <?php the_post_thumbnail( 'lawgit-practice-4' );?>
    <div class="practice-single-title">
        <h2><?php the_title();?></h2>
    </div>
</div>
...
<div class="blog-tags">
    <div class="row">
        <div class="col-md-8 tags">
            <?php if ( !empty(get_the_term_list( $post->ID, 'lawgit_practice_tag')) ): ?>
                <span class="title"><?php esc_html_e( 'Tags', 'lawgit' );?>: </span>
                <?php echo wp_kses_post(get_the_term_list( $post->ID, 'lawgit_practice_tag')); ?>
            <?php endif; ?>
        </div>
        <?php if ( $lawgit_practice_enable_social == 'on' ): ?>
        <div class="col-md-4">
